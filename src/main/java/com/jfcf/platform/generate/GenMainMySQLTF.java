 ...
 * 本身表里的字段就是驼峰的
 */
public class GenMainMySQLTF extends GenerateBase {
	private static Logger log = Logger.getLogger(GenMainMySQLTF.class);
	/**
	 * 配置数据库中药生成的表
	 * 二维数组说明：
	 * 数据源（必须配置数据源）、
...
//	};
	 ...
	 */
    public GenMainMySQLTF(GenConfig config,String[][] tableArr) {
	...
	public List<TableColumnDto> getColunm(String dbName,String tableName)  {
		...
		for (Record record : listColumn) {
			...
			table.setColumn_name_t(column_name);
			table.setColumn_name(column_name);
			table.setColumn_name_upperCaseFirstOne(toUpperCaseFirstOne(column_name));
